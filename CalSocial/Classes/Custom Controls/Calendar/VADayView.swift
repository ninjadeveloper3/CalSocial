...
class VADayView: UIView {
    ...
    func setupDay() {
        ...
        dateLabel.font = UIFont.systemFont(ofSize: 14, weight: UIFont.Weight.ultraLight)
        ...
        let selecteDate = formatter.string(from: day.date)
        
        if selecteDate == currentDate {
            dateLabel.layer.borderWidth = 1.0
            dateLabel.layer.borderColor = UIColor.appThemeBlackColor().cgColor
        }
    ...
    private func setState(_ state: VADayState) {
        ...
        if dayViewAppearanceDelegate?.shape?() == .circle && state == .selected {
            dateLabel.clipsToBounds = true
            dateLabel.layer.cornerRadius = dateLabel.frame.height / 2
            dateLabel.font = UIFont.muliBold(14.0)
        
        } else {
            dateLabel.font = UIFont.muliRegular(14.0)
        }
                
        backgroundColor = dayViewAppearanceDelegate?.backgroundColor?(for: state) ?? backgroundColor
        layer.borderColor = dayViewAppearanceDelegate?.borderColor?(for: state).cgColor ?? UIColor.appThemeBlackColor().cgColor
        layer.borderWidth = 0
